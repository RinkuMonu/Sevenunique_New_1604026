...
function TestimonialCard({ item, featured = false }) {
    const card = (
        <div
            className={`group relative overflow-hidden rounded-[8px] border border-[#cfd5dd] bg-[#f5f6f7] p-4 md:p-5 ${featured ? "min-h-[304px]" : ""
                }`}
        >
    ...
    return (
        <div className="flex items-center">
            {avatars.map((avatar, index) => (
                <div
                    key={avatar.src}
                    className={`relative flex h-11 w-11 items-end justify-center overflow-hidden rounded-full border-2 border-[#f5f6f7] transition-transform duration-300 ease-out group-hover:-translate-y-1 ${index === 0 ? "" : "-ml-2"
                        }`}
                    style={{
...
export default function TestimonialSection({ testimonials }) {
    const sectionLabel = testimonials?.sectionLabel || "Client Testimonials";
    const heading =
        testimonials?.heading || "Get AI results you can stand behind.";
    const description =
        testimonials?.description ||
        "Our work holds up in reviews, in production, and in front of the board.";
    const leftItems = testimonials?.leftItems || leftTestimonials;
    const rightItems = testimonials?.rightItems || rightTestimonials;
    const calloutText =
        testimonials?.calloutText ||
        "Join 500+ companies building AI products with our engineers.";
    const calloutButtonLabel =
        testimonials?.calloutButtonLabel || "Schedule a Call";
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="group relative overflow-hidden rounded-[8px] border border-[#cfd5dd] bg-[#f5f6f7] p-5 transition-colors duration-500 hover:bg-[#d7ebff]">
                            ...
                            <div className="mt-5 w-fit overflow-hidden rounded-[8px] border bg-[#2563eb]">
                                ...
                                >
                                    <span className="relative z-10">{calloutButtonLabel}</span>  
                                </button>
                            </div>
                            </div>
                        </div>
